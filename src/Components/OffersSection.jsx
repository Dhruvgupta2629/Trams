  ...
  return (
    <section id="testimonials" className="px-5 py-10 sm:px-8 lg:px-12">
      <div className="relative pb-6">
        ...
        <div className={`absolute left-0 top-12 hidden h-12 w-12 rotate-45 rounded-[1rem] border md:block ${isDark ? "border-white/10" : "border-zinc-300/80"}`} />
        <h2
        ...
        <div className={`absolute left-0 right-0 bottom-0 h-px ${isDark ? "bg-white/10" : "bg-zinc-200"}`} />
      </div>

      <div className={`divide-y ${isDark ? "divide-white/10" : "divide-zinc-200"}`}>
        {offerItems.map((item, index) => (
          <div
            key={item.index}
            className={`animate-fade-up hover-lift relative grid gap-4 py-7 pl-6 md:grid-cols-[1fr_auto] md:items-center ${isDark ? "text-zinc-100" : "text-zinc-900"}`}
          ...
          >
            ...
            <div className={`text-lg uppercase tracking-[0.12em] ${isDark ? "text-zinc-400" : "text-zinc-700"}`}>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <span className={isDark ? "text-zinc-400" : "text-zinc-500"}>&rarr;</span>
            </div>
          </div>
        ))}
      </div>
